    ...
    <div className={darkMode ? "dark" : ""}>
      <header className="absolute inset-x-0 top-0 z-50">
        ...
        >
          <div className="" />
          <Dialog.Panel
            className={`fixed dark:bg-slate-800 inset-y-0 right-0 z-50 w-full overflow-y-auto px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10 ${
          ...
          >
            ...
            <div className="mt-6 flow-root">
              ...
              >
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <NavLink to={item.route} key={item.name}>
                      <div
                        key={item.name}
                        className={`nav py-3 text-sm font-semibold leading-6 ${
                          darkMode ? "text-cyan-300" : "text-gray-900"
                        } relative w-fit after:block after:content-[''] after:absolute after:h-[3px] after:bg-indigo-600 after:rounded-full after:w-full after:scale-x-0 after:hover:scale-x-100 after:transition after:duration-500 after:origin-center`}
                      >
                        {item.name}
                ...
                <div className="py-6">
                  <button
                    className={`-mx-3 block rounded-lg py-2.5 px-3 text-base font-semibold leading-7 ${
                      darkMode
                        ? "text-cyan-100 hover:bg-indigo-900 transition-all duration-200 ease-in-out"
                        : "text-gray-900 hover:bg-gray-50 transition-all duration-200 ease-in-out"
                    } `}
                    onClick={logout}
                  >
                    Log out
